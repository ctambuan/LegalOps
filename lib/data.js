...
import {
  collection, doc, addDoc, setDoc, getDoc, updateDoc, deleteDoc, deleteField, writeBatch,
...
/* ===================== Company Data · Approval Policy ===================== */
// Thresholds (the USD bands) + the per-department routing matrix. Group-level config: GC writes
// only (firestore.rules). Tools read live via lib/companyData.js, falling back to lib/docgen seeds.

export function listenCfgThresholds(cb) {
  return onSnapshot(doc(DB(), "cfg_thresholds", "bands"), (snap) => cb(snap.exists() ? snap.data() : null));
}
export async function saveCfgThresholds({ low, high }, user) {
  await setDoc(doc(DB(), "cfg_thresholds", "bands"),
    { low: Number(low), high: Number(high), updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() }, { merge: true });
  await audit(user, "save", "cfg_thresholds", "bands", null, null);
}

export function listenCfgApprovals(cb) {
  return onSnapshot(query(collection(DB(), "cfg_approvals"), orderBy("department")), (snap) =>
    cb(snap.docs.map((d) => ({ _id: d.id, ...d.data() })))
  );
}
// One-time load of the workbook routing matrix into the editable collection (GC action).
export async function seedCfgApprovals(departments, user) {
  const db = DB();
  let batch = writeBatch(db), n = 0;
  for (const d of departments) {
    batch.set(doc(db, "cfg_approvals", d.code), {
      department: d.name, departmentCode: d.code,
      admin: d.admin || "", low: d.agree25 || "", mid: d.agree100 || "", high: d.agreeOver || "",
      updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp(),
    }, { merge: true });
    if (++n % 400 === 0) { await batch.commit(); batch = writeBatch(db); }
  }
  await batch.commit();
  await audit(user, "seed", "cfg_approvals", `count:${departments.length}`, null, null);
}
export async function saveCfgApproval(code, patch, user) {
  await setDoc(doc(DB(), "cfg_approvals", code),
    { ...patch, updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() }, { merge: true });
  await audit(user, "save", "cfg_approval", code, null, null);
}

/* ===================== Company Data · Agents (AI & Knowledge) ===================== */
// The agent ROSTER is fixed in code (lib/agentTemplates.js). cfg_agents holds only the GC's per-agent
// OVERRIDES (doc id = preset id): { instruction?, model?, enabled }. GC writes only (rules); all
// allowlisted users read and may run enabled agents. No create/delete of agents — only tuning.
export function listenCfgAgents(cb) {
  return onSnapshot(collection(DB(), "cfg_agents"), (snap) => {
    const m = {}; snap.docs.forEach((d) => (m[d.id] = d.data())); cb(m); // keyed by preset id
  });
}
export async function saveCfgAgentOverride(id, patch, user) {
  await setDoc(doc(DB(), "cfg_agents", id),
    { ...patch, updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() }, { merge: true });
  await audit(user, "tune", "cfg_agent", id, null, null);
}
export async function resetCfgAgent(id, user) {
  await deleteDoc(doc(DB(), "cfg_agents", id));   // drop overrides → back to preset defaults
  await audit(user, "reset", "cfg_agent", id, null, null);
}

/* ===================== Company Data · Policy Library (RAG source) ===================== */
// Policies are chunked on ingest; chunks carry denormalised scope/company/title so the rules can
// read-scope them cheaply and retrieval can cite them. GC writes only; reads are scope-enforced.
import { chunkText } from "./policy";

// GC management list (GC reads all). Non-GC users consume policies only via retrieval (lib/policy.js).
export function listenCfgPolicies(cb) {
  return onSnapshot(query(collection(DB(), "cfg_policies"), orderBy("title")), (snap) =>
    cb(snap.docs.map((d) => ({ _id: d.id, ...d.data() })))
  );
}

export async function addCfgPolicy(meta, text, user) {
  const db = DB();
  const scope = meta.scope === "company" ? "company" : "group";
  const company = scope === "company" ? (meta.company || "") : "";
  const title = (meta.title || "").trim();
  const chunks = chunkText(text);
  const pRef = doc(collection(db, "cfg_policies"));

  let batch = writeBatch(db);
  batch.set(pRef, {
    title, category: meta.category || "", scope, company,
    version: Number(meta.version) || 1, effectiveDate: meta.effectiveDate || "",
    sourceName: meta.sourceName || "", sourceFileId: meta.sourceFileId || "", sourceUrl: meta.sourceUrl || "",
    status: "active", chunkCount: chunks.length,
    createdBy: user.email.toLowerCase(), updatedAt: serverTimestamp(),
  });
  let ops = 1;
  for (let i = 0; i < chunks.length; i++) {
    batch.set(doc(collection(db, "cfg_policies", pRef.id, "chunks")), { ord: i, text: chunks[i], scope, company, title });
    if (++ops % 450 === 0) { await batch.commit(); batch = writeBatch(db); }
  }
  await batch.commit();
  await audit(user, "add", "cfg_policy", pRef.id, null, `chunks:${chunks.length}`);
  return pRef.id;
}

export async function updateCfgPolicyMeta(id, patch, user) {
  await setDoc(doc(DB(), "cfg_policies", id),
    { ...patch, updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() }, { merge: true });
  await audit(user, "save", "cfg_policy", id, null, null);
}

export async function archiveCfgPolicy(id, archived, user) {
  await updateDoc(doc(DB(), "cfg_policies", id),
    { status: archived ? "archived" : "active", updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() });
  await audit(user, archived ? "archive" : "restore", "cfg_policy", id, null, null);
}

/* ===================== Company Data · Risk Register ===================== */
// Operational legal-risk records (read-scoped by company; in-scope makers manage). A risk belongs to a
// company code or "group". `seesAll` (GC/Regional) → whole register; otherwise scope to the user's
// companies + group (single `in` query, sorted client-side to avoid a composite index).
export function listenRisks({ seesAll, companies }, cb) {
  const col = collection(DB(), "risks");
  if (seesAll) {
    return onSnapshot(query(col, orderBy("createdAt", "desc")), (snap) =>
      cb(snap.docs.map((d) => ({ _id: d.id, ...d.data() }))));
  }
  const vals = [...(Array.isArray(companies) ? companies.slice(0, 29) : []), "group"];
  return onSnapshot(query(col, where("company", "in", vals)), (snap) =>
    cb(snap.docs.map((d) => ({ _id: d.id, ...d.data() }))));
}
export async function addRisk(data, user) {
  const ref = await addDoc(collection(DB(), "risks"), {
    ...data, status: data.status || "open",
    createdBy: user.email.toLowerCase(), createdAt: serverTimestamp(), updatedAt: serverTimestamp(),
  });
  await audit(user, "add", "risk", ref.id, null, data.status || "open");
  return ref.id;
}
export async function saveRisk(id, patch, user) {
  await updateDoc(doc(DB(), "risks", id), { ...patch, updatedAt: serverTimestamp() });
  await audit(user, "save", "risk", id, null, patch.status || null);
}
...
// `cfg` carries the live approval matrix + thresholds (Company Data) and per-(year,series) starts:
//   { approvals: {deptName→{admin,low,mid,high}}, thresholds: {low,high}, startSeq }.
export async function createDocNumber(record, user, cfg = {}) {
  const db = DB();
  ...
  const thresholds = cfg.thresholds || undefined; // businessApprovers falls back to defaults
  const starts = cfg.startSeq || {};
  ...
  const result = await runTransaction(db, async (tx) => {
    ...
    const approvers = businessApprovers(
      { department: record.department, category: record.category, docType: record.docType, unbudgeted: record.unbudgeted, usd: record.usdEquivalent },
...
export async function saveDocgenSettings(patch, user) {
  await setDoc(doc(DB(), "docgen_settings", "config"),
    // Strip the legacy approver matrix: it now lives in cfg_approvals (Company Data → Approval Policy).
    { ...patch, approvers: deleteField(), updatedAt: serverTimestamp(), updatedBy: user.email.toLowerCase() }, { merge: true });
