...
// Approval thresholds (USD/annum). Editable in Company Data → Approval Policy (cfg_thresholds);
// these are the seed/fallback values, faithful to the original workbook matrix.
export const DEFAULT_THRESHOLDS = { low: 25000, high: 100000 };

// Map a USD figure onto a band KEY ("low" | "mid" | "high"). Decoupled from any display string so
// editing the threshold numbers can never desync from the routing logic.
export function valueBucketKey(usd, thresholds = DEFAULT_THRESHOLDS) {
  if (usd == null || Number.isNaN(usd)) return "";
  const low = Number(thresholds?.low ?? 25000), high = Number(thresholds?.high ?? 100000);
  if (usd <= low) return "low";
  if (usd < high) return "mid";
  return "high";
}

// Human label for a band, computed from the live thresholds (for previews / stored records).
export function bucketLabel(key, thresholds = DEFAULT_THRESHOLDS) {
  const low = Number(thresholds?.low ?? 25000), high = Number(thresholds?.high ?? 100000);
  const f = (n) => n.toLocaleString("en-US");
  if (key === "low") return `≤ USD ${f(low)} per annum`;
  if (key === "mid") return `USD ${f(low)} < value < USD ${f(high)} per annum`;
  if (key === "high") return `≥ USD ${f(high)} per annum`;
  return "";
}

// Legacy display helper (default thresholds) — retained for backward compatibility.
export function usdValueBucket(usd) {
  return bucketLabel(valueBucketKey(usd), DEFAULT_THRESHOLDS);
}
...
// The four routing slots per department: admin (Administrative Documents) + the three agreement
// value bands. cfg_approvals stores {admin, low, mid, high}; the workbook seed maps
// agree25→low, agree100→mid, agreeOver→high.
export function approverCell(department, approvals = {}) {
  const base = deptByName(department);
  const baseCell = base ? { admin: base.admin, low: base.agree25, mid: base.agree100, high: base.agreeOver } : null;
  const ov = approvals[department];
  if (!baseCell && !ov) return null;
  return { ...(baseCell || {}), ...(ov || {}) };
}

// Business Approvers (O column). `approvals` is keyed by department name → {admin, low, mid, high}
// (Company Data overrides, merged over the workbook defaults); `thresholds` are the live USD bands.
// Computes the band from the USD amount directly, so editing thresholds can never desync routing.
export function businessApprovers({ department, category, docType, unbudgeted, usd }, approvals = {}, thresholds = DEFAULT_THRESHOLDS) {
  // Policy documents are department-agnostic and always require the highest approver.
  if (seriesForType(docType) === "POL") return CLEVEL;
  const cell = approverCell(department, approvals);
  if (!cell) return "";
  if (category === "Administrative Documents") return cell.admin || "";
  if (category === "Agreements or Binding Documents") {
    if (unbudgeted) return cell.high || "";       // unbudgeted → highest approver regardless of value
    if (usd == null) return "";                    // value not yet known → don't guess a route
    return cell[valueBucketKey(usd, thresholds)] || "";
  }
  return "";
}
...
export function buildDocumentNumber(record, seq) {
  ...
  const no = padSeq(seq);
  // Prefer the code carried from the live entity picker (Company Data); fall back to the seed lookup.
  const ent = record.entityCode || entityCode(record.entity);
