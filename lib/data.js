...
// Configured AI agents (name + instruction + model). GC writes only (rules); all allowlisted read
// and may run them. Retrieval from the Policy Library is added in Phase 3.
export function listenCfgAgents(cb) {
  return onSnapshot(query(collection(DB(), "cfg_agents"), orderBy("name")), (snap) =>
    cb(snap.docs.map((d) => ({ _id: d.id, ...d.data() })))
  );
}
export async function addCfgAgent(data, user) {
  const ref = await addDoc(collection(DB(), "cfg_agents"),
    { ...data, status: "active", updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() });
  await audit(user, "add", "cfg_agent", ref.id, null, null);
  return ref.id;
}
export async function saveCfgAgent(id, patch, user) {
  await setDoc(doc(DB(), "cfg_agents", id),
    { ...patch, updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() }, { merge: true });
  await audit(user, "save", "cfg_agent", id, null, null);
}
export async function archiveCfgAgent(id, archived, user) {
  await updateDoc(doc(DB(), "cfg_agents", id),
    { status: archived ? "archived" : "active", updatedBy: user.email.toLowerCase(), updatedAt: serverTimestamp() });
  await audit(user, archived ? "archive" : "restore", "cfg_agent", id, null, null);
}
