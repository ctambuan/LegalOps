...
- OI1. **Drive write authorisation.** RESOLVED (2026-06-01): record-keeping writes go to the configured
  ...
  EXTENDED (2026-06-03): the reviewer can now also **archive superseded documents in-app** — move a file
  out of the Workbench folder into its **Archived** subfolder (a move, not a delete; fully restorable).
  This must act on files the app did not create, which `drive.file` cannot do, so when enabled the
  reviewer's sign-in requests the broad `https://www.googleapis.com/auth/drive` scope — a deliberate
  escalation accepted by the Head of Legal, gated behind `NEXT_PUBLIC_DRIVE_MANAGE` (default off; falls
  back to narrow `drive.file` when off). Enabled in production 2026-06-03. Archive target via
  `NEXT_PUBLIC_DRIVE_ARCHIVE_FOLDER_ID` (defaults to the live Archived folder). See Section 13 and DEPLOY §10.
