...
export async function exportMaster(adopted) {
  const children = [];
  ...
  adopted.forEach((m, i) => {
    ...
    children.push(new Paragraph({ spacing: { after: 40 }, children: [
      new TextRun({ text: `Type: ${CTYPES[m.type] || m.type} · Proposed by ${m.authorName || m.authorEmail} · `
        + `Adopted ${adoptedAt ? adoptedAt.toLocaleDateString() : "—"}${m.baseRef ? " · Ref " + m.baseRef : ""}`,
  ...
  saveAs(blob, `${slug}_Clause_Library_Master_${new Date().toISOString().slice(0, 10)}.docx`);
}
