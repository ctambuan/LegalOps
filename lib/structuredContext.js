...
// `source`: "secretarial" | "compliance" | "all" | "risks". `scope`: { role, companies }.
export async function buildStructuredContext(source, scope) {
  try {
    if (source === "risks") {
      const col = collection(DB(), "risks");
      const seesAll = scope.companies === "all" || ["gc", "regional", "reviewer"].includes(scope.role);
      let risks;
      if (seesAll) risks = (await getDocs(col)).docs.map((d) => d.data());
      else {
        const vals = [...(Array.isArray(scope.companies) ? scope.companies.slice(0, 29) : []), "group"];
        risks = (await getDocs(query(col, where("company", "in", vals)))).docs.map((d) => d.data());
      }
      const open = risks.filter((r) => r.status !== "closed");
      if (!open.length) return "No open risks are recorded in the register for the scope in view.";
      return open.map((r) =>
        `RISK: ${r.title} [${r.company}] — likelihood ${r.likelihood || "?"}, impact ${r.impact || "?"}, status ${r.status || "open"}.`
